      ...
      <td>
        {bodyKind === 'lf-file' ? (
          ...
          <Form.Control
            ...
            rows={2}
            size="sm"
            placeholder="Inline body with log-format vars"
...
const HttpErrorsEditor = ({ entries, onChange }) => {
  ...
  return (
    <>
      {entries.length === 0 ? (
        <p className="text-muted small mb-2">No http-error directives.</p>
      ) : (
  ...
  <Row className="g-2 pt-3">
    <Col md={4}>
      <Form.Group>
        ...
        />
        <Form.Text className="text-muted">Immutable after creation.</Form.Text>
      </Form.Group>
    </Col>
    <Col md={4}>
      <Form.Group>
        <Form.Label>HAProxy section name</Form.Label>
        <Form.Control
          type="text"
          value={draft.name ?? ''}
          onChange={e => update({ name: e.target.value })}
        />
        <Form.Text className="text-muted">Used as the `from NAME` reference.</Form.Text>
      </Form.Group>
    </Col>
    <Col md={4}>
      <Form.Group>
    ...
    <Col md={8}>
      <Form.Group>
        <Form.Label>default-server init-addr (resolution order)</Form.Label>
        <Form.Select
          multiple
...
const ErrorsTab = ({ draft, update, sections }) => (
  <div className="pt-3 d-flex flex-column gap-3">
    <ErrorFilesIdSelect
      label="errorfiles section (named bundle)"
      sections={sections}
      value={draft.useErrorFilesId}
      onChange={v => update({ useErrorFilesId: v })}
      helpText="References one of the named http-errors sections (top-level)."
    />
    <div>
...
const AdvancedTab = ({ draft, update }) => (
  <div className="pt-3">
    <Form.Group>
      ...
      />
      <Form.Text className="text-muted">Appended verbatim inside the defaults block.</Form.Text>
    </Form.Group>
  </div>
);
  ...
  return (
    <Modal show={show} onHide={onCancel} size="xl" backdrop="static">
      <Modal.Header closeButton>
        <Modal.Title>
          {isExisting ? `Edit defaults block: ${block.name}` : 'New defaults block'}
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
        ...
        <Tabs defaultActiveKey="basics" id="defaults-edit-tabs" className="mb-1">
          ...
          <Tab eventKey="timeouts" title="Timeouts">
            <TimeoutsTab draft={draft} update={update} />
          </Tab>
          <Tab eventKey="errors" title="Error pages">
